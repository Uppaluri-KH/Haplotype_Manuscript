...
import re
import sys
df = pd.read_csv("/run/media/administrator/Expansion1/azure_backup/final_vcf/s3_uploaded_snps/1721_samples/PRX/Pharmacogenomics/PGx_Genes_Variants_DP15/{}_final_DP.vcf".format(sys.argv[1], sys.argv[1]), comment= '#', sep = '\t', header=None, low_memory=False)
...
data = pd.read_excel("~/Downloads/Madhu_Updated/RSIDS_POSITION_DBSNP.xlsx")
df = pd.merge(df, data, on = ['CHROM', 'POS', 'REF', 'ALT'], how = 'inner', sort = False)
df2 = pd.read_csv("~/Downloads/Madhu_Updated/13_Genes_Key_Variants_Coordinates.csv", sep = '\t')
...
data = pd.read_excel("~/Downloads/Madhu_Updated/Multiple_Positions.xlsx")
...
mapped_df
mapped_df.to_excel("/run/media/administrator/Expansion1/azure_backup/final_vcf/s3_uploaded_snps/1721_samples/PRX/Pharmacogenomics/PGx_Genes_Variants_DP15/Outputs_16_Dec_2023/{}.xlsx".format(sys.argv[1], sys.argv[1]), index=False)